...
func _run_healthy_level(level_id: int) -> void:
	...
	while true:
		...
		# Reduced from 8 ticks/20s — 3 healthy levels × 30s = 90s was hitting the
		# 120s CI timeout (issue #575). 6 shots still covers 3 wave transitions.
...
func _run_new_towers_showcase() -> void:
	...
	# Reduced to fit 3 healthy levels within the 120s CI timeout (#575).
	for i in 3:
