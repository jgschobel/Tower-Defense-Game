...
func _populate_tower_shop() -> void:
	...
	for tower_id in _shop_tower_ids:
		...
		# Farm towers (gold_per_round > 0) deal no damage — show their economy
		# stat instead. "Schad/Sek 0" was confusing for the Banani-Hof / farm
		# tower which is meant for income, not killing.
		if "gold_per_round" in td and td.gold_per_round > 0:
			dps_label.text = "+%d G/Wälle" % int(td.gold_per_round)
		else:
			dps_label.text = "Schad/Sek %.0f" % (td.damage * td.attack_speed)
...
# Enemy meta helpers (icon, color, name, counter hint, threat tier) live
# in scripts/ui/enemy_meta.gd — extracted from this file as the first
# step of the audit-recommended hud.gd split (was 2,834 lines).
# Local thin wrappers kept ONLY because internal callers in this file
# expected the underscored method names. New code should call EnemyMeta.*
# static methods directly.

func _enemy_icon_texture(enemy_id: String) -> Texture2D:
	return EnemyMeta.icon_texture(enemy_id)


func _enemy_preview_color(enemy_id: String) -> Color:
	return EnemyMeta.preview_color(enemy_id)


func _enemy_counter_hint(enemy_id: String) -> String:
	return EnemyMeta.counter_hint(enemy_id)


func _enemy_threat_tier(enemy_id: String) -> int:
	return EnemyMeta.threat_tier(enemy_id)
...
func _build_wave_preview_chip(enemy_id: String, count: int, is_boss: bool) -> Control:
	var tier_color: Color = EnemyMeta.threat_color(enemy_id)
	var _tier: int = EnemyMeta.threat_tier(enemy_id)
	# Frame panel
...
func _short_name_for_enemy(enemy_id: String) -> String:
	return EnemyMeta.short_name(enemy_id)
...
func _refresh_tower_info() -> void:
	...
	if stats_lbl:
		...
		if td.has_branching_upgrades():
			...
			pip_str += "\n"
		stats_lbl.text = "%sSchade: %.0f   Schad/Sek: %.1f   K.O.: %d\nTempo: %.1f   Reichwiiti: %.0f" % [
			pip_str,
