...
func _ready() -> void:
	...
	# Style each button — was default Godot grey. Now warm dark with
	# gold-tinted hover so they read as "real menu items".
	var vbox: VBoxContainer = get_node_or_null("Panel/VBox")
	if vbox:
		for child in vbox.get_children():
			if child is Button:
				_style_pause_button(child)


func _style_pause_button(btn: Button) -> void:
	var base := StyleBoxFlat.new()
	base.bg_color = Color(0.18, 0.16, 0.14, 1.0)
	base.border_color = Color(0.55, 0.45, 0.20, 0.85)
	...
	pressed.bg_color = Color(0.42, 0.32, 0.12, 1.0)
