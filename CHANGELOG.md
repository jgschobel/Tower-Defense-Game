...
## 2026-06-08 (audit-polish — Wagli-Schub, issue cleanup, ROADMAP ticks)

- feat(gameplay): "Wagli-Schub" drag-to-push active power — 🛒 30g top-bar button; drag finger to push enemies 35 px backward along their path (30g/enemy, max 4/stroke); dust puff VFX; auto-deactivates on lift or bankruptcy. First physical-gesture power in the game. PR #766.
- chore(roadmap): tick Synergie-Combo [x] (shipped PR #758), tick Wagli-Schub [x] (shipped PR #766).
- chore(issues): closed stale playtest issues #137, #328, #330, #380, #417, #465 (superseded by timebox+T2-A fixes); closed #726 (headless CI FPS artifact); closed #762 (OCR misread of Z'nöch am Wäg toast).
- chore(prs): closed permanently-blocked audit-grid PRs #759, #763 (GITHUB_TOKEN can't trigger validate workflow — issue #742).
